package chapters24HR;

import java.awt.*;

class PointTester4D {
		public static void main(String[] args) {
			Point object1 = new Point(11,22);
			Point4D object2 = new Point4D(7,6,64,55);
			
			System.out.println("The 2D point is located at (" + object1.x 
					+ ", " + object1.y + ")");
			System.out.println("\tIt's being moved to (4,13)");
			object1.move(4, 13);
			System.out.println("The 2D point is now at (" + object1.x 
					+ ", " + object1.y + ")");
			System.out.println("\tIt's being moved -10 units on both the x " 
					+ "and y axes");
			object1.translate(-10,-10);
			System.out.println("The 2D point ends up at (" + object1.x
					+ ", " + object1.y + ")\n");
			System.out.println("The 4D point is located at (" + object2.x 
					+ ", " + object2.y + ", " + object2.z + ", " + object2.t + ")");
			System.out.println("\tIt's being moved to (10,22,71,27)");
			object2.move(10,22,71,27);
			System.out.println("The 4D point is now at (" + object2.x
					+ ", " + object2.y + ", " + object2.z + "," + object2.t + ")");
			System.out.println("\tIt's being moved -20 units on the x, y "
					+ "and z axes");
			object2.translate(-20,-20,-20, -20);
			System.out.println("The 4D point ends up at (" + object2.x
					+ ", " + object2.y + ", " + object2.z + ", " + object2.t + ")");
		}
	}

